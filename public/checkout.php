<?php 
	...
	{	
		...
		foreach ($results as $value)
		{
			...
			$done = query($sql, $value["units"], $value["product_id"]);
		}		
		
		//update the cash for that seller
		$sql = "UPDATE users SET cash  = cash + ? WHERE user_id = ? LIMIT 1;";
		$success = query($sql, $total_seller_gain, $seller_id[0]["user_id"]);
	...
	$sql = "UPDATE users SET cash  = cash - ? WHERE user_id = ? LIMIT 1;";
	$sucess = query($sql, $total_cost,$_SESSION["id"]);
